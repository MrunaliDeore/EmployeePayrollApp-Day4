...
public class EmployeeController {
    ...
    * get users details by id
    ...
    public User getById (@PathVariable int id){
        User response = employeeService.getById(id);
        return response;
    }
   ...
   // @Autowired
    UserDTO userDTO;
    @PostMapping("/dtousercreate")
    public ResponseEntity<ResponseDTO> createUser(@Valid @RequestBody UserDTO userDTO) {
        User user1 = new User(userDTO);
        ResponseDTO responseDTO = new ResponseDTO("New User Created Successfully", iEmployeeService.addUser(user1));
        return new ResponseEntity<>(responseDTO, HttpStatus.OK);
    }
